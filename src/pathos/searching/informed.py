...
def astar(
    problem: GoalCostOriented[S, A],
    heuristic: Heuristic[S] = null_heuristic
    ) -> Optional[Node[S, A]]:
        ...
        cost_so_far: Dict[S, float] = {
              start_node.state: start_node.path_cost
        }
        ...
        while frontier:
            ...
            for child in node.expand(problem):
                ...
                if (
                    child.state not in cost_so_far
                    or new_cost < cost_so_far[child.state]
                ):
